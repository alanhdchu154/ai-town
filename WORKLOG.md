...
Last updated: 2026-06-16 10:35 CDT
...
| 7 | Scene-first UI lane is now active: the main world defaults to generated VN-style Scene Mode, while the original Pixi map remains behind the `地圖` toggle. Scene objects are intentionally UI-only hotspots/event seeds for now; do not add durable backend object/event behavior without a proposal because that would affect world continuity and memory. Corrected transparent default-emotion standees now exist for the active core cast (`海`, `天澤`, `一之瀨`, `貓貓`, `祥子`, `真晝`); the next art task is completing consistent multi-emotion render sets, not another Tianze rescue. 2026-06-16 mobile UX patch: selected-character conversation CTA now uses one state model for `先接手 Alan` / `邀請 X` / `直接對話` / busy states, character lookup is view/focus-only instead of secretly moving Alan, and mobile standee/conversation portraits have larger non-clipping containers. cc review `umi/reports/20260616T153202Z-workload.md` passed with caveats; the pre-commit camera-jump nit was fixed. Remaining separate caveat: confirm whether engine `startConversation` still walks both participants to a midpoint instead of anchoring Alan in the principal office. | Alan / Umi / Codex | mobile_cta_flow_patched_engine_anchor_followup |
