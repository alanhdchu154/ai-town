...
Schedule decision: see `docs/paper/SCHEDULE_DECISION.md`. The primary design is
...
- The previous `n>=40/arm` target was not tied to a minimum detectable effect
  ...
  specified.
- Offline power sensitivity now lives under `docs/paper/results/power/` and
...
For legacy forced-pilot evidence only, `--runs 'docs/paper/results/ablation-*'`
