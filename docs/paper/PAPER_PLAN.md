...
> claim → 證據 → 必須守住的邊界的**可稽核版**：[CLAIM_EVIDENCE_MATRIX.md](CLAIM_EVIDENCE_MATRIX.md)。
...
| 原本的段落 | 現在的權威來源 |
|---|---|
| §2 現有證據盤點（誠實版） | [ALAN_HANDOFF.md](ALAN_HANDOFF.md)（一頁 boundary）＋ [CLAIM_EVIDENCE_MATRIX.md](CLAIM_EVIDENCE_MATRIX.md)（claim→證據帳本） |
| §3 實驗設計（Exp 1–4） | [EXPERIMENTS.md](EXPERIMENTS.md)（可執行協議）；長期 ablation 的決定與暫停見 [LONGITUDINAL_EXPERIMENT_PLAN.md](LONGITUDINAL_EXPERIMENT_PLAN.md) + [SCHEDULE_DECISION.md](SCHEDULE_DECISION.md) + [PREREGISTRATION_PROTOCOL.md](PREREGISTRATION_PROTOCOL.md) |
| §4 資料分析計畫（bootstrap / permutation / Cliff's δ / κ / Spearman） | [EXPERIMENTS.md](EXPERIMENTS.md) 各 Exp 的 analyze 步驟 ＋ [scripts/paper/README.md](../../scripts/paper/README.md)（資料契約 + selftest） |
| §5 投稿計畫（tiered：OSF → arXiv → workshop → full） | [SUBMISSION_STRATEGY.md](SUBMISSION_STRATEGY.md)（venue 表 + 解鎖條件 + title 候選） |
| §6 必須對外更正（markers 是規則式、非 LLM-judge） | 已處理：manuscript 現以「deterministic rule-based markers」陳述，LLM-judge 列 future work。邊界見 [REVIEWER_PREMORTEM.md](REVIEWER_PREMORTEM.md) §2 |
| §7 對外宣稱強度（能說 / 不能說） | [ALAN_HANDOFF.md](ALAN_HANDOFF.md) + [CLAIM_EVIDENCE_MATRIX.md](CLAIM_EVIDENCE_MATRIX.md) 結尾的 reviewer-safe 一句話邊界 |
| §9 給 Codex 的第二意見問題 | 已在 review 中處理：因果宣稱限縮、樣本量改 MDE-driven、定位採 OSF/A-path、novelty 防守等，落在 [SCHEDULE_DECISION.md](SCHEDULE_DECISION.md) 與 [REVIEWER_PREMORTEM.md](REVIEWER_PREMORTEM.md) |

> 進度總帳（做到哪、剩哪些 blocker）：[PUBLISH_READY_CHECKLIST.md](PUBLISH_READY_CHECKLIST.md)。
