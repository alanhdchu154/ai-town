...
export default function Game({ view = 'world', onChangeView }: GameProps = {}) {
  ...
  useEffect(() => {
    const onCharacterNavigation = (event: Event) => {
      ...
      if (detail.travel) {
        if (!activeHuman) {
          setSceneMessage(
            `Alan 目前離校；已切到 ${targetName} 所在場景。按「接手 Alan」後，再讓他親自過去。`,
          );
  ...
  const switchScene = (nextId: SchoolLocationId) => {
    ...
    if (!humanPlayer) {
      ...
      setSceneMessage(
        `已切換觀察視角到：${nextScene.labelZh}。Alan 仍然離校，按「接手 Alan」才會回到校園。`,
      );
      return;
    }
    focusOn(nextScene.position, 1.2);
  ...
  const focusAlan = () => {
    ...
    focusOn(ClassroomCenter, 1.02);
    setSceneMessage('Alan 目前離校；按「接手 Alan」才會回到校園。');
  };
      ...
      >
        <div className="giis-topbar">
          ...
          <button
            ...
            onClick={focusAlan}
            title={alanPlayerForFocus ? '把鏡頭移到 Alan 所在位置' : 'Alan 離校中；按「接手 Alan」才會回到校園。'}
          >
            找到 Alan
          </button>
