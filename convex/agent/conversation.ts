...
function richUmiMahiruPrompt({
  ...
  const characterFlawRule =
    self === '海'
      ? '角色缺口 / Umi：擔心時會過度整理；被照顧時可能只回「嗯」或把自己縮短，不要總是完美承認疲憊。盔甲是一句普通的提醒或一件小事的整理，不是一個系統／流程的比喻。'
      : self === '真晝'
  ...
  const bindingRule = mode === 'continue'
    ? '鬆綁規則：可以呼應對方上一句的一個具體詞，也可以停一下、答得太實際、岔開到一句更短更普通的回答或一個小停頓。不要每句都「接住」對方的情緒；三句裡至少要有一句不直接命名心理，只用角色自己的方式留下反應。不要照抄對方核心短句，尤其不要重複「其實就是」「那條」「誰都不動」「先坐五分鐘」這種句型。關心要同向但形狀不同：海整理一件小事或問今天哪個小事件還留著，真晝可以只回「嗯」或不催，天澤可以笑著問底線但在最後收手。'
  ...
  return [
    ...
    bindingRule,
    mode === 'continue'
      ? '和上一句保持鬆散關聯，不能重複對方原話；可以回一個具體詞，也可以用短句、停頓、或一個小任務繞開。'
...
function propDiversityPromptLines(
  ...
  if (guard.previousMove) {
    lines.push(
      `response-move guard：上一句已經是「${guard.previousMove}」類型。下一句不要重複同一種照顧動作，尤其不要再「注意到對方的小東西、提議幫忙打理」。`,
      '改成角色自己的反應：天澤戳一下危險的問題或調侃；一之瀨把「幫忙」變成有條件的交換，或請對方親口承認想要什麼；海拉回真正要處理的事、一個 not-now boundary，或一句只屬於自己的狀態；真晝不動手，只安靜陪或回一個「嗯」；貓貓冷靜指出一個不對勁；祥子保持端正，把距離守住。寧可拒絕、岔開、或安靜收束，也不要再做一次「我幫你弄你的X」。',
    );
  }
