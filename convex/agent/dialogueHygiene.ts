...
export function hasFirstPersonStageDirectionLeak(line: string) {
  return line
    .replace(/（[^）]{1,100}）|\([^)]{1,100}\)/g, '')
    .split(/(?<=[，,。！？!?；;])/)
    .some((clause) => isStageDirectionClause(clause));
}
...
function isStageDirectionClause(clause: string) {
  ...
  return (
    /^(?:我)(?:輕輕|慢慢|先|再|又|正|正在|剛|剛剛|剛才|剛才只是|默默|順手|得去|去)?(?:合上|關上|關掉|放下|看向|走到|靠回|拿起|起身|伸手|握住|碰了|碰到|推開|推過來|按住|移開|坐下|站起|轉身|低頭|抬頭|停下|停住|靠近|退開|盯著|把手機|把[^，,。！？!?；;]{0,18}(?:放下|轉過去|拿起|推開|推過來|按住|移開|合上|關上|關掉|收起|遞過去|蓋好|劃掉|圈掉|收進|放進|推到|倒了|倒掉|倒出|拉上|拉開|拉嚴|端過來|端來))/.test(
      withoutLeadIn,
    ) ||
    /^我(?:的)?(?:手|指尖|手指|掌心|視線|目光|腳步)[^，,。！？!?；;]{0,24}(?:停在|碰|握|移開|靠近|按|放|拉|收)/.test(
      withoutLeadIn,
    ) ||
