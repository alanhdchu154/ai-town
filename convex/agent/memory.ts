...
export function buildResiduePrompt(
  ...
  lines.push(
    ...
    transcript,
    ``,
    `Write at most ONE short line of 情緒殘留 in Traditional Chinese：這次對話在你心裡留下的一道具體痕跡——${other} 實際做了或說了什麼，那讓你（${self}）感覺到什麼。`,
    `規則：`,
    ...
    `- 不要每次都抓一個微小的身體動作當殘留（喉結、睫毛、眨眼、停頓半拍、尾音上揚、袖口的灰）；只有那個細節真的承載了意義才用。其他時候改成留意：對方的選擇、語氣、沉默、迴避了什麼、做了什麼決定，或老實說這次其實沒留下什麼。`,
