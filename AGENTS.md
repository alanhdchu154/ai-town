...
- Use Alan's current two-loop flow: Central Umi / Codex defines direction, acceptance criteria, and source-of-truth context; cc does deep VS Code Claude Code / code-mode work for complex Underworld logic; Codex reviews, verifies, accepts/rejects/narrows, then updates the right source-of-truth files. For broad or risky changes, ask cc for an independent findings-first review of current diff/status before treating the work as complete.
...
- After every meaningful cc/Codex loop, run the source-of-truth sync gate: active worker task -> `umi/workload.md`, recent evidence -> `WORKLOG.md`, durable direction / v0.1 acceptance change -> `docs/giis-v0.1-roadmap.md`, cross-project risk or Alan attention -> Central Umi files. Do not let a VS Code cc chat or Codex chat be the only record of an accepted result, changed blocker, or next action.
